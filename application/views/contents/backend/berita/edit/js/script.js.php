...
<script>
    ...
    $('#input_ubah_gambar').on('change', function() {
        $('#show-cover').show();
    ...
    const $update = async (form) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                axios.post(BASE_URL + 'update/<?= @$data->uuid ?>', formData)
                    .then(res => {
                        Swal.fire({
                            icon: 'success',
                            title: 'Success!',
...
<script>
    ...
    const ajax_select2_kategori_id = (status) => {
        ...
        $('#form_ubah select#select_ubah_kategori_id')
            .append(new Option("<?= @$data->nama_kategori ?>", "<?= @$data->kategori_id ?>", true, true))
            .trigger('change')
            .trigger({
                type: 'select2:select',
                params: {
    ...
    const ajax_select2_tag = (status) => {
        ...
        <?php foreach ($tags as $k => $v) : ?>
            $('#form_ubah select#select_ubah_tags')
                .append(new Option("<?= $v->tag_name ?>", "<?= $v->tag_id ?>", true, true));
        <?php endforeach ?>
