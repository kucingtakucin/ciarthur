...
class Halaman extends MY_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model($this->_path . 'Crud', 'Crud_halaman');   // Load CRUD model
	...
	private function _validator($status = 'tambah')
	{
		...
		if ($status === 'tambah' || $this->Crud_halaman->num_rows([
			'judul' => post('judul'),
			'is_active' => '1',
			'uuid !=' => $status === 'ubah' ? post('uuid') : null
		])) $is_unique = '|is_unique[halaman.judul]';
		else $is_unique = '';
	...
	public function insert()
	{
		...
		elseif ($this->input->method() === 'post') :
			...
			$halaman_id = $this->Crud_halaman->insert(
				[
					'uuid' => uuid(),
	...
	public function update($uuid = null)
	{
		...
		if ($this->input->method() === 'get') :
			...
			$config = [
				...
				'uri_segment' => $this->_path,
				'breadcrumb' => [
					'Backend', 'Website', ucwords($this->_name), 'Edit'
				],
